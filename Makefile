...
export CLI_VERSION := $(if $(filter testnet,$(CLUSTER)),1.14.17,$(if $(filter mainnet,$(CLUSTER)),1.9.29,$(if $(filter localnet,$(CLUSTER)),1.9.29,$(error Unknown cluster name: $(CLUSTER)))))
...
build: 
	@$(MAKE) install-deps CLI_VERSION=1.14.17 ANCHOR_VERSION=0.25.0		
